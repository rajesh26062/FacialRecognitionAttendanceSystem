...
class Attendance:
    ...
    def __init__(self, root):
        ...
        # title section
        title_lb1 = Label(bg_img, text="Ai based Attendance Management System", font=("verdana", 20, "bold"), bg="navyblue",
                          fg="white")
        ...
        # Student id
        studentId_label = Label(left_frame, text="Student ID:", font=("verdana", 12, "bold"), fg="navyblue",
                                bg="white")
        ...
        # Student Roll
        student_roll_label = Label(left_frame, text="Roll Number:", font=("verdana", 12, "bold"), fg="navyblue",
                                   bg="white")
        ...
        # Studnet Name
        student_name_label = Label(left_frame, text="Student Name:", font=("verdana", 12, "bold"), fg="navyblue",
                                   bg="white")
        ...
        # Attendance
        student_attend_label = Label(left_frame, text="Status:", font=("verdana", 12, "bold"), fg="navyblue",
                                     bg="white")
        ...
        self.attendanceReport_left.heading("Attend", text="Attendance")
        ...
        # Exprot button
        update_btn = Button(btn_frame, command=self.exportCsv, text="Export CSV", width=12, font=("verdana", 12, "bold"),
                            fg="white", bg="navyblue")
        update_btn.grid(row=0, column=1, padx=6, pady=8, sticky=W)

        # Update button
        del_btn = Button(btn_frame, command=self.action, text="Update", width=12, font=("verdana", 12, "bold"), fg="white",
                         bg="navyblue")
        ...
        # Right Label Frame
        right_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE, text="Details",
                                 font=("verdana", 12, "bold"), fg="navyblue")
    ...
    def update_data(self):
        if self.var_id.get() == "" or self.var_roll.get == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                Update = messagebox.askyesno("Update", "Do you want to update this student's attendance?",
                                             parent=self.root)
                if Update > 0:
    ...
    def delete_data(self):
        if self.var_id.get() == "":
            ...
            try:
                delete = messagebox.askyesno("Delete", "Do you want to delete this student's attendance?", parent=self.root)
                if delete > 0:
                    conn = mysql.connector.connect(user='root', password='Pass@123', host='localhost',
                                                   database='face_recognition',
                                                   port=3306)
    ...
    def action(self):
        if self.var_id.get() == "" or self.var_roll.get == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success", "All records saved in the database!", parent=self.root)
            except Exception as es:
                messagebox.showerror("Error", f"Due to: {str(es)}", parent=self.root)
